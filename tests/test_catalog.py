...
def test_write_read_roundtrip(tmp_path):
    store = make_store(tmp_path, Artifact("point", JsonModelCodec(Point)))
    obj = Point(x=1, y=2)
    path = store.write("point", "grp", "r1", obj)
    assert path.exists()
    assert store.read("point", "grp", "r1") == obj
